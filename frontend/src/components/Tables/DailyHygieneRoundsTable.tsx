      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        {!isLoading && (
          <div className="px-4 py-4">
            <PaginateSpanHeader
              totalPageItemCount={hygieneData.to}
              TotalItemCount={hygieneData.total}
            />
          </div>
        )}
        <div className="max-w-full overflow-x-auto">
          <table className="w-full table-auto">
            ...
            <tbody>
              {hygieneData?.data?.length > 0 &&
                hygieneData?.data?.map((input: any, key) => (
                  <tr key={key} className="even:bg-gray-2 dark:even:bg-boxdark">
                    ...
                    <td className="border-b border-[#eee] py-2 px-1 dark:border-strokedark">
                      <p className="text-xs text-black dark:text-white">
                        {t(input?.deviation_type?.name_key)} /{" "}
                        {t(input?.deviation_code?.name_key)}
                      </p>
                    </td>
                    ...
                    <td className="border-b border-[#eee] py-2 px-1 dark:border-strokedark">
                      <p className="text-sm text-black dark:text-white">
                        {t(input?.risk_category.name)}
                      </p>
                    </td>
                    <td className="border-b border-[#eee] py-2 px-1 dark:border-strokedark">
                      <p className="text-sm text-black dark:text-white">
                        {t(input?.danger?.name_key)}
                      </p>
                    </td>
                    <td className="border-b border-[#eee] py-2 px-1 dark:border-strokedark">
                      <p className="text-sm text-black dark:text-white">
                    ...
                    <td className="border-b border-[#eee] py-2 px-1 dark:border-strokedark">
                      <p className="text-xs text-black dark:text-white">
                        {input?.cleared_before}
                      </p>
                    </td>
                    <td className="border-b border-[#eee] py-2 px-1 dark:border-strokedark">
                      <p className="text-sm text-black dark:text-white">
                        {input?.implemented_by}
                      </p>
                    </td>
                          ...
                          onClick={() => {
                            setOpenModal(true),
                              setModalQueryData({
                                ...
                                modalData: {
                                  index: key,
                                  endPoint: "hygienerounds/" + input.id,
                                  action: "DELETE",
                                  token: user.token,
                                  title:
                                    t(input.deviation_type?.name_key) +
              ...
              {!isLoading && hygieneData?.data?.length == 0 && (
                <tr>
                  <td
          ...
          <div className="pl-3 pt-2">
            ...
            <Spinner />
            <Paginate
              currentPage={hygieneData.current_page}
              totalPages={hygieneData.last_page}
...
export default DailyHygieneRounds;
